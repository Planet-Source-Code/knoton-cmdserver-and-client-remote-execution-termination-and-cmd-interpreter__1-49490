...
'Change the Path
Public Sub CD(sPath As String)
If sPath = "." Then
sPath = Mid(CurDir, 1, InStrRev(CurDir, "\", 3))
End If

sPath = Trim(sPath)

If SetCurrentDirectory(sPath) Then
    If Mid(sPath, 2, 1) <> ":" Then
        sPath = Replace(CurDir & sPath, ">", "")
    End If
...
'Check what kind of command it was and take action accordingly
Public Function GetCMD(strCMD As String) As String
If UCase(Left(strCMD, 2)) = "CD" Then
    CD Mid(strCMD, 4)
    GetCMD = "CURDIR" & CurDir
ElseIf UCase(Trim(strCMD)) = "GETRUNAPP" Then
    GetCMD = GetRunningApps
ElseIf UCase(Left(strCMD, 5)) = "REXEC" Then
    strCMD = Replace(UCase(strCMD), "REXEC ", "")
    RExec strCMD
    GetCMD = "Attempting to run " & strCMD
ElseIf UCase(Left(strCMD, 5)) = "RTERM" Then
    strCMD = Replace(UCase(strCMD), "RTERM ", "")
    RTerminate strCMD
    GetCMD = "Attempting to terminate " & strCMD
ElseIf UCase(Left(strCMD, 5)) = "REXIT" Then
    Form1.Server.Close
    End
Else
    strCMD = Environ$("COMSPEC") & " /c " & strCMD
    GetCMD = GetCommandOutput(strCMD, True, True)
End If
End Function
